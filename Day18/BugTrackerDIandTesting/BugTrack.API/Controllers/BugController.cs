...
namespace BugTrack.API.Controllers
{
    ...
    public class BugController : ControllerBase
    {
        ...
        [HttpGet("async/{id}")]
        public async Task<IActionResult> GetBugByIdAsync(int id)
        {
        ...
        [HttpPost("async")]
        public async Task<IActionResult> AddBugAsync([FromBody] BugResquestDTO bugDto)
        {
            if (bugDto == null || string.IsNullOrWhiteSpace(bugDto.Title))
                return BadRequest(new { message = "Invalid bug data." });
            await _service.AddBugAsync(bugDto);
            return StatusCode(201, new { message = "Bug created successfully." });
        }

        [HttpPut("async/{id}")]
        public async Task<IActionResult> UpdateBugAsync(int id, [FromBody] BugResquestDTO bugDto)
        {
            if (bugDto == null || string.IsNullOrWhiteSpace(bugDto.Title))
                return BadRequest(new { message = "Invalid bug data." });
            var existingBug = await _service.GetBugByIdAsync(id);
            if (existingBug == null)
                return NotFound(new { message = $"Bug with ID {id} not found." });
            await _service.UpdateBugAsync(id, bugDto);
            return NoContent(); // 204 No Content
        }
        [HttpDelete("async/{id}")]
        public async Task<IActionResult> DeleteBugAsync(int id)
        {
            var existingBug = _service.GetBugById(id);
            if (existingBug == null)
                return NotFound(new { message = $"Bug with ID {id} not found." });
