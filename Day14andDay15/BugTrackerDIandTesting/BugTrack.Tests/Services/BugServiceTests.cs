...
namespace BugTrack.Tests.Services
{
    public class BugServiceTests
    {
        ...
        public void GetAllBugs_ShouldReturnListOfBugResponseDTO()
        {
            ...
            {
                new Bug { Id = 1, Title = "Bug 1", Description = "Description 1", Status = "Open", CreatedOn =DateTime.UtcNow ,Project = new Project {ProjectName = "Project B" } },
                new Bug { Id = 2, Title = "Bug 2", Description = "Description 2", Status = "Closed", CreatedOn =DateTime.UtcNow ,Project = new Project {ProjectName = "Project B" } }
            };
            _bugRepositoryMock.Setup(repo => repo.GetAll()).Returns(bugs.Select(b => new Bug
            {
                Id = b.Id,
                Title = b.Title,
